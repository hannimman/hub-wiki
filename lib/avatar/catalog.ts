// 아바타 v2 카탈로그 — SVG 레이어드 아바타 (TEST_With_Hannim 데모에서 이식)
// 좌표계: viewBox 0 0 320 400. 머리 중심 (160,120) r56 / 눈 (140,110),(180,110) / 손 좌(103,252) 우(217,252) / 바닥 y=360
// 규칙: 아이템 svg 안에 id 속성 금지(복제 렌더 시 충돌). 소매는 sleeve-l/sleeve-r 마커 그룹.
// 서버(가격검증·sanitize)와 클라(상점 UI)가 함께 import하는 순수 데이터 모듈.

export type Slot = { id: string; name: string; zoom: string };
export type AvatarItem = { id: string; name: string; price: number; svg: string; rigid?: boolean };
export type FaceOption = { id: string; name: string; svg: string };
...
export const SLOTS: Slot[] = [
  ...
  { id: "decoL", name: "애완동물 1", zoom: "8 268 100 104" },
  { id: "decoR", name: "애완동물 2", zoom: "212 268 100 104" },
];

// ---------- 공용 도형 헬퍼 ----------
function hairCap(c: string) {
...
export const FACE_OPTIONS: Record<string, FaceOption[]> = {
  eyes: [
    {
...
export function findFace(part: string, optionId: string | null | undefined): FaceOption | null {
  ...
  return (FACE_OPTIONS[part] || []).find((o) => o.id === optionId) ?? null;
}
export const SLOT_IDS = SLOTS.map((s) => s.id);
// itemId → {slotId, item} 역인덱스 (구매 API에서 가격을 서버 카탈로그로 확정할 때 사용)
export const ITEM_INDEX: Record<string, { slotId: string; item: AvatarItem }> = {};
for (const s of SLOTS) for (const it of ITEMS[s.id] || []) ITEM_INDEX[it.id] = { slotId: s.id, item: it };
